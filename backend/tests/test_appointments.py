...
def test_cancel_appointment(client, mock_patient_id, mock_doctor_id):
    ...
    cancel_res = client.patch(f"/api/appointments/{appt_id}/cancel")
...
def test_reschedule_appointment(client, mock_patient_id, mock_doctor_id):
    ...
    resched_fail = client.patch(f"/api/appointments/{appt_id}/reschedule", json={
        "date": "2026-09-25",
        "time": "15:00:00"
    })
    ...
    resched_ok = client.patch(f"/api/appointments/{appt_id}/reschedule", json={
        "date": "2026-09-26",
        "time": "16:00:00"
    })
...
def test_list_patient_appointments(client, mock_patient_id, mock_doctor_id):
    ...
    response = client.get(f"/api/appointments/patient/{mock_patient_id}")
...
def test_get_appointment_not_found(client):
    ...
    response = client.get(f"/api/appointments/{uuid.uuid4()}")
